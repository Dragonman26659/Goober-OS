...
uint8_t physicalMemoryBitmap[NUM_PAGE_FRAMES / 8]; // TODO: make dynamic
...
void memMapPage(uint32_t virtualAddr, uint32_t physAddr, uint32_t flags) {
    ...
    pt[pdIndex] = physAddr | PAGE_FLAG_PRESENT | flags;
    mem_num_vpages++;
    invalidate(virtualAddr);
